...
# kernel path: /tmp/torchinductor_wangke61/q4/cq4lbplwwed6uqs2pdf2g25idcsifdtuy7oeukkayc4jwfhhv4uv.py
# Topologically Sorted Source Nodes: [a_rms], Original ATen: [aten._to_copy, aten.pow, aten.mean, aten.add, aten.rsqrt, aten.mul]
# Source node to ATen node mapping:
#   a_rms => add, convert_element_type, convert_element_type_1, mean, mul, pow_1, rsqrt
# Graph fragment:
...
def triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_0(in_ptr0, out_ptr1, xnumel, r0_numel, XBLOCK : tl.constexpr, R0_BLOCK : tl.constexpr):
    ...
    for r0_offset in range(0, r0_numel, R0_BLOCK):
        ...
        tmp8 = 4096.0
        tmp9 = (tmp4 / tmp8)
        tmp10 = 9.999999747378752e-06
        tmp11 = tmp9 + tmp10
        tmp12 = libdevice.rsqrt(tmp11)
        tmp13 = tmp7 * tmp12
        tmp14 = tmp13.to(tl.float32)
        tl.store(out_ptr1 + (r0_1 + 4096*x0), tmp14, r0_mask)
''', device_str='cuda')
...
def call(args):
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_0.run(arg0_1, buf1, 4096, 4096, stream=stream0)
        del arg0_1
        buf2 = empty_strided_cuda((4096, 4096), (4096, 1), torch.bfloat16)
        # Topologically Sorted Source Nodes: [a_rms, matmul], Original ATen: [aten._to_copy, aten.pow, aten.mean, aten.add, aten.rsqrt, aten.mul, aten.mm]
        extern_kernels.mm(buf1, reinterpret_tensor(arg1_1, (4096, 4096), (1, 4096), 0), out=buf2)
        del arg1_1
        del buf1
    return (buf2, )
