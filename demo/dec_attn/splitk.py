...
from cutedsl_kernels import DAttnSplit1, AttnReduce1
...
# ncu --set full -o dec_attn_splitk_4_4096 -f --launch-skip=17
if __name__ == '__main__':
    ...
    # O[0, 0, 0, :] = torch.arange(0, 128, dtype=torch.float32)
    # O[0, 0, 1, :] = torch.arange(0, 128, dtype=torch.float32)
    # Osum[0, :, 0] = torch.arange(0, 2, dtype=torch.float32)
    # print(O)
    compiled_reduce(*red_tensors)
    torch.cuda.synchronize()
    # print(O)
    # print(Osum)
    # finished = O.sum(dim=2) / Osum.sum(dim=1)[..., None]
    # print(finished[0, 0, :16])
    # print(O_final[0, 0, :128])
    # print(O_final[0, 0, :16])
    ...
    if not IS_NCU:
        ...
        my_rmse = get_rmse(ref64, O_final.to(ref64.dtype))
        sdpa_rmse = get_rmse(ref64, o_sdpa.to(ref64.dtype))
        print(f'{ref_rmse=}, {my_rmse=}')
        print(f'more precise: {ref_rmse > my_rmse}')
