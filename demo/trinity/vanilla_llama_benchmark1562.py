...
def kernel_0(
    ...
    for p in range(0, 1024, BLOCK_P):
        ...
        C_sum = ((1 * C_sum) + tl.sum(C_exp, axis=2, dtype=tl.float32))
        ...
        O = (tl.dot(C_exp, temp_9.to(tl.float32)) + (1 * O))
    ...
    tl.store(O2_ptr + offset_8, tl.reshape(temp_10, (16, 128)).to(tl.float16), mask=mask_6)
...
def run_forward():
    ...
    trinity_ms = do_bench(lambda: forward(K_cache, O2, V_cache, WK, WQ, WV, X))
    print(trinity_ms)
