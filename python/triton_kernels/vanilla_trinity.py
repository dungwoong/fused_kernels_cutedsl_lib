...
def kernel_0(
    ...
    # NOTE: it's doing QKV multiplication here
    for k in range(0, 4096, BLOCK_K):

        # NOTE load X, all programs loop through (X[:, :])
        ...
        # NOTE load K and V in a similar way
    ...
    Q2 = tl.expand_dims(Q1, 1) # NOTE M, Block_N

    K2 = tl.expand_dims(K1, 1)

    V2 = tl.expand_dims(V1, 1)
    ...
    for p in range(0, 1024, BLOCK_P):
        ...
        # NOTE pids are organized in row-major order so n//BLOCK_N is just M which is head dim/d model
        # so they store to K_cache[m, 1008:1024, 0:128] so yeah they are just assigning each head to a process I'm guessing
        # TODO check the grid dims to confirm
        # Also, I hope this store gets hoisted.
        ...
        temp_4 = tl.load(K_cache_ptr + offset_6, mask=mask_4, other=0.0)
        temp_5 = tl.permute(temp_4, (0, 2, 1))
        C_exp = tl.exp(tl.dot(Q, temp_5).to(tl.float32))
        C_sum = (tl.sum(C_exp, axis=2, dtype=tl.float32) + (1 * C_sum))
        offset_7 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None, None] * V_cache_stride0 + (p + tl.arange(0, BLOCK_P))[None, :, None] * V_cache_stride1 + (tl.arange(0, 128))[None, None, :] * V_cache_stride2
        mask_5 = (p_indices < P+M)[None, :, None]
        temp_6 = tl.load(V_cache_ptr + offset_7, mask=mask_5, other=0.0)

        # NOTE they don't adjust for precision or online softmax at all here
        # I think softmax is one example where numerical equivalence does not mean it's a good transformation
        O = ((O * 1) + tl.dot(C_exp, temp_6.to(tl.float32)))
    # Skipped empty sloop with dummy body

    # NOTE this is just the store, no computation here
    O = (O / C_sum[:, :, None])
    temp_7 = tl.permute(O, (1, 0, 2))
    offset_8 = (tl.arange(0, 16))[:, None] * O2_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * O2_stride1
    n_indices = n + tl.arange(0, BLOCK_N)
    mask_6 = (n_indices < N)[None, :]
    tl.store(O2_ptr + offset_8, tl.reshape(temp_7, (16, 128)).to(tl.float16), mask=mask_6)
