...
def matmul_kernel_tma_persistent(a_desc, b_desc, c_desc,  #
    ...
    for tile_id in tl.range(start_pid, num_tiles, NUM_SMS, flatten=True, warp_specialize=WARP_SPECIALIZE):
        ...
        for ki in range(k_tiles):
            ...
            sum_acc += tl.sum(a * a, axis=1).to(tl.float32)
        
        rms = tl.rsqrt((sum_acc / K) + 1e-5)[:, None]
        accumulator = accumulator * rms
...
def rmsnorm_linear_tma_persistent(a, b, warp_specialize: bool):
    # Check constraints.
