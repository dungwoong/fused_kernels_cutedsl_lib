...
@cute.jit
def make_A_reduction_tensor(tiled_mma: cute.TiledMma, tile_m: int, tile_n: int, dtype: Type[cutlass.Numeric]):
    s2r_r_shape = tiled_mma.partition_shape_A(
            (tile_m, tile_n)
        )
    # mma fragment is ((2, 2, 2), nrows, ncols), so we want (2, nrows) so we can reduce across cols
    layout = cute.make_layout((2, s2r_r_shape[1]))
    return cute.make_rmem_tensor(layout, dtype)
...
def warp_sum_row_mma_layout(
    ...
    so e.g. you reduce threads 0-3 for the first row
    """
...
@cute.jit
def elemwise_rsqrt_plus_eps(row_reduce_regs: cute.TensorSSA, dim: int, epsilon: cutlass.Numeric) -> cute.TensorSSA:
    out = cute.make_rmem_tensor(row_reduce_regs.shape, row_reduce_regs.dtype)
    for i in cutlass.range_constexpr(cute.size(row_reduce_regs.shape)):
        out[i] = cute.math.rsqrt(row_reduce_regs[i] / dim + epsilon, fastmath=True)
    return out.load()
...
class GemmSM90:
    ...
    def __init__(
        ...
        self.a_layout, self.b_layout, self.b1_layout = None, None, None
        self.a_smem_layout_staged = None
        self.b_smem_layout_staged = None
        self.b1_smem_layout_staged = None
    ...
    @cute.jit
    def __call__(self, a: cute.Tensor, b: cute.Tensor, b1: cute.Tensor, c: cute.Tensor, eps: cutlass.Float32):
        # Populate fields
        self.populate_dtypes_and_layouts(a, b, b1, c)
        ...
        tma_atom_b1, tma_tensor_b1, _b1bytes = self._get_tma_load_and_tensors_incr_bytes(b1, self.b1_smem_layout_staged, (self.cta_tile_shape_mnk[1], self.cta_tile_shape_mnk[2]), self.mcast_ctas_b, self.dtype)
        self.tma_ab_load_bytes = _abytes + _bbytes + _b1bytes
        ...
        self.kernel(
            ...
            tma_tensor_a, tma_tensor_b, tma_tensor_b1,
            self.tiled_mma,
            self.a_smem_layout_staged, self.b_smem_layout_staged, self.b1_smem_layout_staged,
            ts_params,
    ...
    def kernel(self,
               ...
               mA: cute.Tensor, mB: cute.Tensor, mB1: cute.Tensor,
               tiled_mma: cute.TiledMma,
               a_smem_layout_staged: cute.ComposedLayout, b_smem_layout_staged: cute.ComposedLayout, b1_smem_layout_staged: cute.ComposedLayout,
               tile_sched_params: ParamsBase,
               cluster_layout_mnk: cute.Layout,
               epi_smem_layout: cute.ComposedLayout,
               epi_copy: cute.CopyAtom,
               epi_mC: cute.Tensor, eps: cutlass.Float32,
               ):
        ...
        sB1 = self.get_smem_field(storage, 'sB1', b1_smem_layout_staged)
        ...
        if warp_idx >= self.ab_load_warp_id: # Producer
            ...
            if warp_idx == self.ab_load_warp_id: # Only TMA warp enters
                ...
                while work_tile.is_valid_tile:
                    ...
                    ab_producer_state = self.produce_mainloop(k_iters, ab_pipeline, ab_producer_state, tma_atom_a, tma_atom_b, tma_atom_b1, mA, sA, mB, sB, mB1, sB1, tile_coord_mnkl, block_in_cluster_coord_mnk, cluster_layout_mnk, a_mcast_mask, b_mcast_mask) # TODO
        ...
        if warp_idx < self.ab_load_warp_id: # Consumer
            ...
            accumulators_b1 = cute.make_rmem_tensor(acc_shape, self.acc_dtype)
            ...
            # NOTE we init the acc after so this will never work
            ...
            while work_tile.is_valid_tile:
                ...
                ab_consumer_state, ab_release_state, accumulate_O, tiled_mma = self.gemm_middle_loop(
                    tidx, self.gemm_n_prologue, k_iters, tiled_mma, reduction_acc,
                    accumulators, accumulators_b1, ab_pipeline, 
                    ab_consumer_state, ab_release_state, 
                    sA, sB, sB1, accumulate_O, wait_num=self.gemm_n_prologue)
                if cutlass.const_expr(self.gemm_n_prologue > 0):
                    ab_release_state = self.gemm_end_release(self.gemm_n_prologue, ab_pipeline, ab_release_state)

                # Epilogue ##################################################
                self.epilogue(tiled_mma, reduction_acc, epi_mC, epi_copy, sD, accumulators, accumulators_b1, tile_coord_mnk, tidx, warp_idx, eps)
    ...
    @cute.jit
    def epilogue(self, tiled_mma, reduction_acc, epi_mC, epi_copy, sD, accumulators, accumulators_b1, tile_coord_mnk, tidx, warp_idx, eps):
        ...
        tRS_rAccb1 = tiled_copy_r2s.retile(accumulators_b1)
        ...
        row_scale(accumulators_b1, reduction_acc)
        ...
        for epi_idx in cutlass.range_constexpr(epi_tile_num):
            for epi_v in cutlass.range_constexpr(size_tRS_rD):
                ...
                tRS_rD[epi_v] = fast_silu(tRS_rAcc[idx]) * tRS_rAccb1[idx] # * reduction_acc[idx1]
    ...
    @cute.jit
    def gemm_middle_loop(self, tidx: Int32, start_iter: Int32, end_iter: Int32, tiled_mma: cute.TiledMma, reduction_acc: cute.Tensor, accumulators: cute.Tensor, accumulators_b1: cute.Tensor, pipe: pipeline.PipelineAsync, read_state: pipeline.PipelineState, release_state: pipeline.PipelineState, sA: cute.Tensor, sB: cute.Tensor, sB1: cute.Tensor, accumulate_O: bool, wait_num: Int32):
        for k in cutlass.range(start_iter, end_iter, unroll=1):
            ...
            mma.accumulating_gemm_rs(tidx, tiled_mma, a_regs, sB1, accumulators_b1, read_state, accumulate_O, -1)
    ...
    # -------------------------------------
    def populate_dtypes_and_layouts(self, a: cute.Tensor, b: cute.Tensor, b1: cute.Tensor, c: cute.Tensor):
        assert a.element_type == b.element_type == b1.element_type == c.element_type == self.dtype
        self.a_layout = utils.LayoutEnum.from_tensor(a)
        self.b_layout = utils.LayoutEnum.from_tensor(b)
        self.b1_layout = utils.LayoutEnum.from_tensor(b1)
        self.c_layout = utils.LayoutEnum.from_tensor(c)
        assert self.a_layout == utils.LayoutEnum.ROW_MAJOR, 'col mjr a not supported yet'
        self.k_dim = cute.size(a, mode=[1]) # need this for rowmean
    ...
    def populate_smem_layouts(self):
        ...
        self.b1_smem_layout_staged = sm90_utils.make_smem_layout_b(
            self.b_layout, self.cta_tile_shape_mnk, self.dtype, self.ab_stage,
        )
    ...
    def populate_shared_storage(self):
        ...
        self.add_memrange(SharedStorage, 'sB1', self.dtype, self.b1_smem_layout_staged, self.buffer_align_bytes)
