...
class Kernel:
  ...
  def __call__(self, a: cute.Tensor, b: cute.Tensor, c: cute.Tensor):
    ...
    tiled_mma_46 = mma.get_tiled_mma(cutlass.BFloat16, True, True, cutlass.Float32, 128, 256, False)
    tiled_mma_68 = mma.get_tiled_mma(cutlass.BFloat16, True, True, cutlass.Float32, 128, 256, False)
    ...
    self.kernel(a, b, c, sA_layout, sB_layout, sC_layout, acc_tiled_mma, tiled_mma_46, tiled_mma_68, c_tma_atom_1, c_tma_tensor_1, a_tma_atom_2, a_tma_tensor_2, b_tma_atom_3, b_tma_tensor_3).launch(grid=[1, 2, 66], block=384, cluster=(1, 2, 1))

  @cute.kernel
  def kernel(self, a: cute.Tensor, b: cute.Tensor, c: cute.Tensor, sA_layout, sB_layout, sC_layout, acc_tiled_mma, tiled_mma_46, tiled_mma_68, c_tma_atom_1, c_tma_tensor_1, a_tma_atom_2, a_tma_tensor_2, b_tma_atom_3, b_tma_tensor_3):
    ...
    if warpidx_ >= 0 and warpidx_ < 8:
      ...
      for sched_idx in cutlass.range(cute.arch.block_idx()[2], 256, 66):
        ...
        for k in cutlass.range(0, 64, 1):
          ...
          mma.accumulating_gemm_ss(tidx_, tiled_mma_46, sA, sB, acc, state_c, state_c, acc_accumulate, 0)
          acc_accumulate = True
          pipe.consumer_release(state_c)
          state_c.advance()
        newacc = elementwise.relu_f32(acc)
        store.mma_epilogue_tma(tiled_mma_68, c_tma_tensor_1, c_tma_atom_1, sC, acc, 128, 256, sched_coord[0], sched_coord[1], tidx_, warpidx_, cutlass.Float32)
