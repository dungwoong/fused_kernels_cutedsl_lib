...
class Kernel:
  ...
  def __call__(self, a: cute.Tensor, b: cute.Tensor, c: cute.Tensor):
    ...
    tiled_mma_175900149853729448841910127731546233028 = mma.get_tiled_mma(cutlass.BFloat16, True, True, cutlass.Float32, 128, 16, False)
    tiled_mma_214187693496772308229471528600267805266 = mma.get_tiled_mma(cutlass.BFloat16, True, True, cutlass.Float32, 128, 16, True)
    tiled_mma_18152616313786870134839950968157013098 = mma.get_tiled_mma(cutlass.BFloat16, True, True, cutlass.Float32, 128, 256, True)
    tiled_mma_140365442237733824375301860020953604465 = mma.get_tiled_mma(cutlass.BFloat16, True, True, cutlass.Float32, 128, 256, False)
    ...
    self.kernel(a, b, c, sA_layout, sB_layout, sC_layout, acc_tiled_mma, tiled_mma_175900149853729448841910127731546233028, tiled_mma_214187693496772308229471528600267805266, tiled_mma_18152616313786870134839950968157013098, tiled_mma_140365442237733824375301860020953604465, c_tma_atom_1, c_tma_tensor_1, a_tma_atom_2, a_tma_tensor_2, b_tma_atom_3, b_tma_tensor_3).launch(grid=[1, 2, 66], block=384, cluster=(1, 2, 1))

  @cute.kernel
  def kernel(self, a: cute.Tensor, b: cute.Tensor, c: cute.Tensor, sA_layout, sB_layout, sC_layout, acc_tiled_mma, tiled_mma_175900149853729448841910127731546233028, tiled_mma_214187693496772308229471528600267805266, tiled_mma_18152616313786870134839950968157013098, tiled_mma_140365442237733824375301860020953604465, c_tma_atom_1, c_tma_tensor_1, a_tma_atom_2, a_tma_tensor_2, b_tma_atom_3, b_tma_tensor_3):
    ...
    if warpidx_ >= 0 and warpidx_ < 8:
      ...
      for sched_idx in cutlass.range(cute.arch.block_idx()[2], 64, 66):
        ...
        sum_acc = reduction.make_mma_A_reduction_tensor(tiled_mma_175900149853729448841910127731546233028, 128, 16, cutlass.Float32)
        for k in cutlass.range(0, 64, 1):
          pipe.consumer_wait(state_c, pipe.consumer_try_wait(state_c))
          a_regs = mma.copy_a_wgmma(tidx_, tiled_mma_214187693496772308229471528600267805266, sA[None, None, state_c.index], 128, 64, cutlass.BFloat16)
          mma.accumulating_gemm_rs(tidx_, tiled_mma_18152616313786870134839950968157013098, a_regs, sB, acc, state_c, acc_accumulate, -1)
          acc_accumulate = True
        ...
        store.mma_epilogue_tma(tiled_mma_140365442237733824375301860020953604465, c_tma_tensor_1, c_tma_atom_1, sC, acc_scaled, 128, 256, sched_coord[0], sched_coord[1], tidx_, warpidx_, cutlass.Float32)
