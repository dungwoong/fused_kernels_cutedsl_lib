...
class Kernel:
  ...
  def __call__(self, mQ: cute.Tensor, mK: cute.Tensor, mV: cute.Tensor, mO: cute.Tensor):
    ...
    tiled_mma_285 = mma.get_tiled_mma(cutlass.BFloat16, True, False, cutlass.Float32, 128, 128, True)
    tiled_mma_303 = mma.get_tiled_mma(cutlass.BFloat16, True, True, cutlass.Float32, 128, 128, False)
    ...
    self.kernel(mQ, mK, mV, sQ_layout, sK_layout, sV_layout, mO, acc_scaled_epi_smem_layout, tiled_mma_44, tiled_mma_41, pv_acc_tiled_mma, tiled_mma_279, tiled_mma_285, tiled_mma_303, mO_tma_atom_1, mO_tma_tensor_1, mQ_tma_atom_2, mQ_tma_tensor_2, mK_tma_atom_3, mK_tma_tensor_3, mV_tma_atom_4, mV_tma_tensor_4).launch(grid=[132, 1, 1], block=384, cluster=[1, 1, 1])

  @cute.kernel
  def kernel(self, mQ: cute.Tensor, mK: cute.Tensor, mV: cute.Tensor, sQ_layout, sK_layout, sV_layout, mO: cute.Tensor, acc_scaled_epi_smem_layout, tiled_mma_44, tiled_mma_41, pv_acc_tiled_mma, tiled_mma_279, tiled_mma_285, tiled_mma_303, mO_tma_atom_1, mO_tma_tensor_1, mQ_tma_atom_2, mQ_tma_tensor_2, mK_tma_atom_3, mK_tma_tensor_3, mV_tma_atom_4, mV_tma_tensor_4):
    ...
    if warpidx_ >= 0 and warpidx_ < 8:
      ...
      for sched_idx in cutlass.range(cute.arch.block_idx()[0], 1024, 132):
        ...
        for k in cutlass.range(0, 32, 1):
          ...
          p_acc_scaled = cute.make_rmem_tensor_like(acc_279, cutlass.Float32)
          p_acc_scaled.store(elementwise.const_mul(acc_279.load(), 0.12751743082459868))
          ...
          rt_801 = conversion.cvt_f16(p_exp_frgA, cutlass.BFloat16)
          sK_pstate.advance()
          sK_cstate.advance()
          sV_pipe.consumer_wait(sV_cstate, sV_pipe.consumer_try_wait(sV_cstate))
          mma.accumulating_gemm_rs(tidx_, tiled_mma_285, rt_801, layout.transpose_view(sV), pv_acc, sV_cstate, pv_acc_accumulate, -1)
        ...
        store.mma_epilogue_tma(tiled_mma_303, mO_tma_tensor_1_slice, mO_tma_atom_1, acc_scaled_epi_smem, acc_scaled, 128, 128, sched_coord[1], 0, tidx_, warpidx_, cutlass.Float32)
