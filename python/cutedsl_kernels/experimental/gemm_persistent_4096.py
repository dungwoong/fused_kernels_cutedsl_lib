...
class Kernel:
  ...
  def __call__(self, a: cute.Tensor, b: cute.Tensor, c: cute.Tensor):
    ...
    self.kernel(a, b, c, sA_layout, sB_layout, sC_layout, acc_tiled_mma, c_tma_atom_1, c_tma_tensor_1, a_tma_atom_2, a_tma_tensor_2, b_tma_atom_3, b_tma_tensor_3).launch(grid=[132, 1, 1], block=384)
      ...
      if warpidx_ == 8:
        ...
        for sched_idx in cutlass.range(cute.arch.block_idx()[0], 512, 132):
          ...
          for k in cutlass.range(0, 64, 1):
            ...
            shared.tma_copy(a_tma_atom_2, a_tma_tensor_2, sA, 128, 64, sched_coord[0], k, pipe, state_p)
            shared.tma_copy(b_tma_atom_3, b_tma_tensor_3, sB, 256, 64, sched_coord[1], k, pipe, state_p)
            state_p.advance()