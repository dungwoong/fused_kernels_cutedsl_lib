...
class Kernel:
  ...
  def __call__(self, a: cute.Tensor, b: cute.Tensor, b1: cute.Tensor, c: cute.Tensor):
    ...
    acc1_tiled_mma = mma.get_tiled_mma(cutlass.BFloat16, True, True, cutlass.Float32, 128, 128, False)
    tiled_mma_7745190238682731477956539034971092474 = mma.get_tiled_mma(cutlass.BFloat16, True, True, cutlass.Float32, 128, 16, True)
    tiled_mma_225088012365968477071473228193797689902 = mma.get_tiled_mma(cutlass.BFloat16, True, True, cutlass.Float32, 128, 128, True)
    tiled_mma_6961395426930584500177275686706438343 = mma.get_tiled_mma(cutlass.BFloat16, True, True, cutlass.Float32, 128, 128, True)
    tiled_mma_320562985924688310207334438753674067840 = mma.get_tiled_mma(cutlass.BFloat16, True, True, cutlass.Float32, 128, 128, False)
    ...
    self.kernel(sA_layout, sB_layout, sB1_layout, a, b, b1, c, final_acc_epi_smem_layout, acc_tiled_mma, acc1_tiled_mma, tiled_mma_7745190238682731477956539034971092474, tiled_mma_225088012365968477071473228193797689902, tiled_mma_6961395426930584500177275686706438343, tiled_mma_320562985924688310207334438753674067840, c_tma_atom_1, c_tma_tensor_1, a_tma_atom_2, a_tma_tensor_2, b_tma_atom_3, b_tma_tensor_3, b1_tma_atom_4, b1_tma_tensor_4).launch(grid=[132, 1, 1], block=384, cluster=[1, 1, 1])

  @cute.kernel
  def kernel(self, sA_layout, sB_layout, sB1_layout, a: cute.Tensor, b: cute.Tensor, b1: cute.Tensor, c: cute.Tensor, final_acc_epi_smem_layout, acc_tiled_mma, acc1_tiled_mma, tiled_mma_7745190238682731477956539034971092474, tiled_mma_225088012365968477071473228193797689902, tiled_mma_6961395426930584500177275686706438343, tiled_mma_320562985924688310207334438753674067840, c_tma_atom_1, c_tma_tensor_1, a_tma_atom_2, a_tma_tensor_2, b_tma_atom_3, b_tma_tensor_3, b1_tma_atom_4, b1_tma_tensor_4):
    ...
    if warpidx_ >= 0 and warpidx_ < 8:
      ...
      for sched_idx in cutlass.range(cute.arch.block_idx()[0], 1024, 132):
        ...
        for k in cutlass.range(0, 64, 1):
          ...
          a_regs = mma.copy_a_wgmma(tidx_, tiled_mma_7745190238682731477956539034971092474, sA[None, None, sA_cstate.index], 128, 64, cutlass.BFloat16)
          mma.accumulating_gemm_rs(tidx_, tiled_mma_225088012365968477071473228193797689902, a_regs, sB, acc, sA_cstate, acc_accumulate, -1)
          acc_accumulate = True
          mma.accumulating_gemm_rs(tidx_, tiled_mma_6961395426930584500177275686706438343, a_regs, sB1, acc1, sA_cstate, acc1_accumulate, -1)
        ...
        store.mma_epilogue_tma(tiled_mma_320562985924688310207334438753674067840, c_tma_tensor_1, c_tma_atom_1, final_acc_epi_smem, final_acc, 128, 128, sched_coord[0], sched_coord[1], tidx_, warpidx_, cutlass.Float32)
