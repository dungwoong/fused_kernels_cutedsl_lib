...
class Kernel:
  ...
  def __call__(self, a: cute.Tensor, b: cute.Tensor, c: cute.Tensor):
    ...
    tiled_mma_55686862120449371446032110272644256199 = mma.get_tiled_mma(cutlass.BFloat16, True, True, cutlass.Float32, 128, 16, True)
    ...
    self.kernel(sA_layout, sB_layout, a, b, c, acc_scaled_epi_smem_layout, tiled_mma_55686862120449371446032110272644256199, acc_tiled_mma, c_tma_atom_1, c_tma_tensor_1, a_tma_atom_2, a_tma_tensor_2, b_tma_atom_3, b_tma_tensor_3).launch(grid=[132, 1, 1], block=384, cluster=[1, 1, 1])

  @cute.kernel
  def kernel(self, sA_layout, sB_layout, a: cute.Tensor, b: cute.Tensor, c: cute.Tensor, acc_scaled_epi_smem_layout, tiled_mma_55686862120449371446032110272644256199, acc_tiled_mma, c_tma_atom_1, c_tma_tensor_1, a_tma_atom_2, a_tma_tensor_2, b_tma_atom_3, b_tma_tensor_3):
    ...
    if warpidx_ >= 0 and warpidx_ < 8:
      ...
      for sched_idx in cutlass.range(cute.arch.block_idx()[0], 128, 132):
        ...
        for k in cutlass.range(0, 64, 1):
          ...
          a_regs = mma.copy_a_wgmma(tidx_, tiled_mma_55686862120449371446032110272644256199, sA[None, None, sA_cstate.index], 128, 64, cutlass.BFloat16)
        ...
        store.mma_epilogue_tma(acc_tiled_mma, c_tma_tensor_1, c_tma_atom_1, acc_scaled_epi_smem, acc_scaled, 128, 256, sched_coord[0], sched_coord[1], tidx_, warpidx_, cutlass.Float32)
