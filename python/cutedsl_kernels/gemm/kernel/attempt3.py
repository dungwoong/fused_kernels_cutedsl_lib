...
class GemmSM90:
    ...
    def kernel(self,
        ...
        if warp_idx < self.ab_load_warp_id: # Consumer
            ...
            # Swapping these causes register spills, likely because PTXAS can't optimize the accumulator usage
            accumulators = cute.make_rmem_tensor(acc_shape, self.acc_dtype)
            accumulators2 = cute.make_rmem_tensor(acc_shape, self.acc_dtype)
            if cutlass.const_expr(self.gemm_n_transition > 0):
                accumulators2 = cute.make_rmem_tensor(acc_shape, self.acc_dtype)

            work_tile = tile_scheduler.initial_work_tile_info()
            # Dispatch prologue GEMMs
            # Precondition: gemm n transition is less than k
            accumulate_O = False
            if cutlass.const_expr(self.gemm_n_transition > 0):
                ab_consumer_state, tiled_mma, accumulate_O = self.gemm_start_loop(tidx, self.gemm_n_transition, tiled_mma, accumulators, ab_pipeline, ab_consumer_state, sA, sB, accumulate_O)
            ...
            while work_tile.is_valid_tile:
                ...
                ab_consumer_state, ab_release_state, accumulate_O, tiled_mma = self.gemm_middle_loop(
                    tidx, self.gemm_n_transition, k_iters, tiled_mma, 
                    accumulators, ab_pipeline, 
                    ab_consumer_state, ab_release_state, 
                    sA, sB, accumulate_O, wait_num=self.gemm_n_transition)
                
                if cutlass.const_expr(self.gemm_n_transition > 0):
                    ab_release_state = self.gemm_end_release(self.gemm_n_transition, ab_pipeline, ab_release_state) # wait group 0
                accumulate_O = False # for next iter

                # Swap the accumulators
                epi_accumulators = accumulators
                if cutlass.const_expr(self.gemm_n_transition > 0):
                    accumulators, accumulators2 = accumulators2, accumulators
                ...
                if cutlass.const_expr(self.gemm_n_transition > 0):                    
                    # Dispatch next GEMMs
                    # You don't actually need the next tile info for the mainloop
                    if work_tile.is_valid_tile:
                        ab_consumer_state, tiled_mma, accumulate_O = self.gemm_start_loop(tidx, self.gemm_n_transition, tiled_mma, accumulators, ab_pipeline, ab_consumer_state, sA, sB, accumulate_O)

                # Epilogue ##################################################
                self.epilogue(tiled_mma, epi_mC, epi_copy, sD, epi_accumulators, tile_coord_mnk, tidx, warp_idx)
    ...
    def epilogue(self, tiled_mma, epi_mC, epi_copy, sD, accumulators, tile_coord_mnk, tidx, warp_idx):
        ...
        for epi_idx in cutlass.range_constexpr(epi_tile_num):
            ...
            epilogue_barrier.arrive_and_wait() # Don't start next stmatrix until wait_group is done
