...
class GemmSM90:
    """
    Remake GEMM with helper functions
    """
    def __init__(
        self,
    ...
    def populate_shared_storage(self):
        SharedStorage = shared.get_smem_struct()
        shared.add_barrier_array(SharedStorage, 'mainloop_pipeline_barriers', self.ab_stage * 2)
        shared.add_memrange(SharedStorage, 'sA', self.dtype, self.a_smem_layout_staged, self.buffer_align_bytes)
        shared.add_memrange(SharedStorage, 'sB', self.dtype, self.b_smem_layout_staged, self.buffer_align_bytes)
        shared.add_memrange(SharedStorage, 'sD', self.dtype, self.epi_smem_layout_staged, self.buffer_align_bytes)
        # no need for epi_bytes since we never reuse sA or sB anymore
